...
export class ProfileChangeModal implements OnInit {
    ...
    public async saveModal() {
      ...
      try {
        let currentUsersJson = await this.storage.getData("users");
        let currentUsers: Array<User> = JSON.parse(currentUsersJson);
        let userToUpdate = currentUsers.find(user => user.email == this.currentUser.email);
        ...
        if(userToUpdate) {
            ...
            this.saveDataToStorage(currentUsers);
